...
main() {
    if [[ "$(az storage container exists --auth-mode login --name "${AZURE_BLOB_CONTAINER_NAME}" --query exists --output tsv)" == "false" ]]; then
        ...
        az storage container --auth-mode login create --name "${AZURE_BLOB_CONTAINER_NAME}" > /dev/null
        az storage container --auth-mode login set-permission --name "${AZURE_BLOB_CONTAINER_NAME}" --public-access container > /dev/null
    fi
